...
try { await stat(runtime); } catch { console.error('WebView2Loader.dll is missing from the native release output.'); process.exit(1); }
await rm(output, { recursive: true, force: true });
await mkdir(output, { recursive: true });
await copyFile(binary, join(output, 'Taskmaster.exe'));
await copyFile(runtime, join(output, 'WebView2Loader.dll'));
